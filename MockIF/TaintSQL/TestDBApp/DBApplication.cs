    ...
    public partial class PexTestClass
    {
        ...
        public int filterMaritalStatus2()
        {
            ...
            try
            {
                ...
                string cmd_statusSearch = String.Format(sql, DBMS.processData(status, "00000001", sql, true));
                MockSqlCommand myCommand2 = new MockSqlCommand(cmd_statusSearch, conn);
                MockSqlDataReader result = myCommand2.ExecuteReader();
                Console.WriteLine(result.records.Count);
                count = result.records.Count;
                if (count == 4) PexAssert.ReachEventually("2");
                while (result.Read())
                {
                    Console.WriteLine("NAME :" + result[0]);
                    Console.WriteLine("NAME_tag:" + result[1]);
                    Console.WriteLine("MA :" + result[10]);
                    Console.WriteLine("MA_tag:" + result[11]);
                }
            }
            catch (Exception e)
            {
